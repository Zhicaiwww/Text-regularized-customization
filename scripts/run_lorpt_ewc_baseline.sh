...
export INSTANCE_DIR="/data/zhicai/code/da-fusion/extracted/aircraft-16"
export CLASS_PRIOR_DIR="dataset/prior_real/samples_dog/dog" # prior preserving dataset dir
export CLASS_CAPTION_DIR="dataset/prior_real/samples_dog/caption.txt" # caption of prior preserving dataset dir, .g.: 
...
for _lora_rank in "${lora_rank[@]}"; do
  ...
  export OUTPUT_DIR="outputs/checkpoints/aircraft/ewc_reg/baseline_lora_rank_${lora_rank}"
  CUDA_VISIBLE_DEVICES=6 python training_scripts/train_lora_w_ti.py \
    ...
    --reg_prompts="photo of an aircraft" \
    --ti_reg_type="decay"
done
