...
export CLASS_PRIOR_DIR="dataset/prior_gen/samples_cat/cat" # prior preserving dataset dir
export CLASS_CAPTION_DIR="dataset/prior_gen/samples_cat/caption.txt" # caption of prior preserving dataset dir, e.g.: 
...
for seed in "${seeds[@]}"; do 
  for _norm_reg_loss_weight in "${norm_reg_loss_weights[@]}"; do
    for _lora_rank in "${lora_rank[@]}"; do
      ...
      export OUTPUT_DIR="outputs/checkpoints/cat/ewc_reg_shareTI/ewc_gen_weight_${norm_reg_loss_weight}_rank_${lora_rank}_seed_${seed}"
