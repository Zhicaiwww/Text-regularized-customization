...
def train(concept: str):
    ...
    caption = f"'<new1> <new2> {concept.replace('_',' ')}'" if not use_BLIP else f'data/{concept}.txt'
            ...
            for norm_k_scale in norm_k_list:
                for norm_v_scale in norm_v_list:
                    try:
                        ...
                        subprocess.check_output(shlex.split(f"python train.py --base configs/custom-diffusion/finetune_addtoken2.yaml -t --gpus 6,7 --resume-from-checkpoint-custom Stable-diffusion/sd-v1-4-full-ema.ckpt --caption {caption} --datapath {data_path} --modifier_token <new1>+<new2> --name {concept} --no-test --reg_k_scale {reg_k_scale} --norm_k_scale {norm_k_scale} --reg_v_scale {reg_v_scale} --norm_v_scale {norm_v_scale} --postfix reg_{reg_k_scale}-{reg_v_scale}_scale{norm_k_scale}-{norm_v_scale}_{tag} --reg_prompt_file {reg_prompt_file} --new_prompt {new_prompt} --repeat 50"))
