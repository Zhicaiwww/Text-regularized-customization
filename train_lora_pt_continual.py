...
from lora_diffusion import (
    ...
    add_lora_params_to_model_by_lox,
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="full",
        help="full | self | cross | cross+self",
    )
...
def main(args):
    ...
    if args.resume_ti_embedding_path is not None:
        ...
        apply_learned_embed_in_clip(
            ...
            text_encoder,
            tokenizer,
            token=args.placeholder_token.split("+")[
                :-1
            ],  # the last placeholder needs to be learned
            idempotent=True,
        )
    else:
        tok_dict = None
    ...
    text_encoder.get_input_embeddings().weight.data[-1] = torch.normal(
        0, 0.014, torch.Size([768])
    )  # Initialize the last placeholder
    ...
    if args.resume_ti_embedding_path is not None:
        ...
        add_lora_params_to_model_by_lox(
            unet, lora, target, ranks, filter_crossattn_str=args.filter_crossattn_str
        )
        ...
        for idx in range(int(len(loras_unet_prev) / 2)):
            lora_prev_matrix.append(
                torch.matmul(
                    loras_unet_prev[2 * idx].float(),
                    loras_unet_prev[2 * idx + 1].float(),
                ).to("cuda")
            )
    else:
            ...
            # region [Unet and text encoder training: begin learning with unet and text encoder]
                ...
                if args.resume_ti_embedding_path is not None:
                    ...
                    for idx, module in enumerate(to_reg_params["lox_loras"]):
                        lora_now = torch.matmul(
                            module.lora_up.weight, module.lora_down.weight
                        )
                        _c_lora_loss.append(
                            torch.norm(torch.mul(lora_prev_matrix[idx], lora_now), p=2)
                        )
                    if len(_c_lora_loss) != 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        if args.output_format == "safe" or args.output_format == "both":
                            ...
                            save_safeloras_with_embeds(
                                ...
                                tok_dict=tok_dict,
                                loras_unet_prev=loras_unet_prev,
                            )
