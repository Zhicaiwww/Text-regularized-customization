...
from visualization.utlis import visualize_images
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name",
        type=str,
        default="up_blocks.2.attentions.1.transformer_blocks.0.attn2",
    )
    ...
    for i in range(len(attn_map)):
        ...
        if i == 40:
            ...
            for j in range(10):
                attn_map = cond_attn_map[:, :, j].unsqueeze(-1).cpu().numpy()
                ax[j].imshow(map)
                # no axis for subplot
                ax[j].axis("off")
